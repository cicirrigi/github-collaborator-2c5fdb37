...
interface IncludedServicesCardV3Props {
  className?: string;
}

export function IncludedServicesCardV3({ className = '' }: IncludedServicesCardV3Props) {
  return (
    <div className={`relative ${className}`}>
      ...
      >
        ...
        <div className='grid grid-cols-4 gap-4'>
          {INCLUDED_SERVICES.slice(0, 8).map(service => (
        ...
        <div className='text-center mt-5 pt-4 border-t border-white/10'>
          <p className='text-amber-200/50 text-xs tracking-wide'>
            {INCLUDED_SERVICES.length} services included at no extra cost
          </p>
        </div>
      </div>
    </div>
...
export type { IncludedServicesCardV3Props };
