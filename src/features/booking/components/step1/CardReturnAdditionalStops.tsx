...
export function CardReturnAdditionalStops() {
  const { tripConfiguration, setReturnAdditionalStops } = useBookingState();
  const { returnAdditionalStops } = tripConfiguration;
  ...
  const handleAddStop = () => {
    ...
    setReturnAdditionalStops([...returnAdditionalStops, newStop]);
  };

  const handleRemoveStop = (index: number) => {
    const updatedStops = returnAdditionalStops.filter((_, i) => i !== index);
    setReturnAdditionalStops(updatedStops);
  };

  const handleStopChange = (index: number, address: string) => {
    const updatedStops = returnAdditionalStops.map((stop, i) =>
  ...
  return (
    <div className='vl-card-flex' style={{ height: 'auto' }}>
      <CardHeader
        icon={ArrowRightLeft}
        title='Return Additional Stops'
        subtitle='Optional stops along your return journey'
      />
      <div className='vl-card-inner space-y-2'>
        {/* Existing Return Stops - Ultra Compact */}
        {returnAdditionalStops.map((stop, index) => (
          <div key={index} className='bg-white/3 rounded p-2 border border-amber-200/10'>
            <div className='flex items-center gap-1.5 mb-1.5'>
              ...
              <span className='text-amber-100/80 text-xs font-light'>Return Stop {index + 1}</span>
              <button
        ...
        {/* Add Return Stop Button - Mini */}
        {returnAdditionalStops.length < 3 && (
          <button
          ...
          >
            ...
            <span className='text-amber-100/80 text-xs font-light'>Add Return Stop</span>
          </button>
        )}

        {/* Info - Mini */}
        {returnAdditionalStops.length === 0 && (
          <div className='bg-white/3 rounded p-2 border border-amber-200/10 text-center'>
            <div className='text-amber-100/60 text-xs font-light'>Add return journey stops</div>
