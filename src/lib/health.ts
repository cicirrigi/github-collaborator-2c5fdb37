...
async function checkFileSystemAccess() {
  ...
  try {
    ...
    const criticalPaths = ['src', 'src/app', 'src/components', 'src/lib', 'docs'];
...
async function checkConfigurationFiles() {
  ...
  try {
    ...
    const configFiles = ['package.json', 'next.config.ts', 'tailwind.config.ts', 'tsconfig.json'];

    for (const configFile of configFiles) {
      await fs.default.access(configFile);
    }
