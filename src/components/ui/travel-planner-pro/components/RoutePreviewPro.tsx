...
export const RoutePreviewPro = ({ className }: RoutePreviewProProps) => {
  // Date reale din Zustand store
  const { tripConfiguration } = useBookingState();
  const { pickup, dropoff, additionalStops } = tripConfiguration;
  ...
  return (
    <div className={className}>
      ...
      <div className='space-y-4'>
        ...
        {/* Route Summary - cu date reale */}
        <div className='text-xs text-neutral-500 text-center'>
          {pickup && dropoff ? (
            <div className='space-y-1'>
              <p className='text-neutral-300'>
                <strong>{pickup.address}</strong> → <strong>{dropoff.address}</strong>
              </p>
              {additionalStops.length > 0 && (
                <p className='text-neutral-400'>
                  +{additionalStops.length} additional stop{additionalStops.length > 1 ? 's' : ''}
                </p>
              )}
            </div>
          ) : (
