...
import { BOOKING_CONFIG, TIME_SLOTS } from '@/components/ui/travel-planner/constants';
import { type Stop, type TimeSlot, type TravelPlan } from '@/components/ui/travel-planner/types';
...
export const useTravelPlanner = (
  ...
  const updateField = useCallback(<K extends keyof TravelPlan>(key: K, value: TravelPlan[K]) => {
  ...
  const handlers = useMemo(
    () => ({
      ...
      setPickup: (date: Date, time?: TimeSlot | null) => {
        setPlan(prev => {
          const updated = { ...prev, pickupDate: date };
