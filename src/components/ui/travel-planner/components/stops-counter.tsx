...
import { StopsCounterProps } from '../types';
...
export const StopsCounter = ({
  ...
  return (
    <div className={cn('space-y-3', className)}>
      ...
      {/* Counter Controls */}
      <div className="flex items-center justify-center gap-4 p-4 rounded-lg bg-gray-50 dark:bg-gray-800/50">
        {/* Decrease Button */}
        <button
        ...
        <div className="flex flex-col items-center gap-1">
          <span className={getDisplayStyle()}>
            {value}
          </span>
      ...
      {max > 0 && (
        <div className="space-y-1">
          ...
          <div className="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-1.5">
            <div
              className="bg-gradient-to-r from-[#CBB26A] to-[#D4AF37] h-1.5 rounded-full transition-all duration-300"
      ...
      {value > 0 && (
        <p className="text-xs text-gray-600 dark:text-gray-400 text-center">
          {value === max 
            ? `Maximum ${max} stops reached`
            : `Add up to ${max - value} more stops`
          }
        </p>
      )}
    </div>
