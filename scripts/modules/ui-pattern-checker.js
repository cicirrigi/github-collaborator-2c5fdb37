...
class UIPatternChecker {
  ...
  async checkUIPatternCompliance() {
    ...
    const components = fs.readdirSync(this.uiDir, { withFileTypes: true })
    ...
    const complianceRate = totalComponents > 0 ? (compliantComponents / totalComponents * 100).toFixed(1) : 0

    console.log(`📊 UI Pattern Compliance: ${compliantComponents}/${totalComponents} (${complianceRate}%)`)
  ...
  checkComponentFileStructure(filePath, componentName) {
    ...
    const patterns = [
      ...
      { pattern: /export interface \w+Props/, message: 'Missing Props interface export' },
      { pattern: /export function \w+/, message: 'Missing main component export' },
      { pattern: /JSX\.Element/, message: 'Consider using JSX.Element return type' },
      { pattern: /className\?:/, message: 'Missing className prop for customization' },
      { pattern: /designTokens|brandConfig/, message: 'Should integrate designTokens or brandConfig' },
      { pattern: /cva\(/, message: 'Should use class-variance-authority (cva) for variants' },
    ]
  ...
  checkTypesFileStructure(filePath, componentName) {
    ...
    const typePatterns = [
      { pattern: new RegExp(`export type ${componentName}Variant`), message: 'Consider adding Variant type' },
      { pattern: new RegExp(`export type ${componentName}Size`), message: 'Consider adding Size type' },
  ...
  checkHookFile(filePath, componentName) {
    ...
    const hookPatterns = [
      { pattern: new RegExp(`export function use${componentName}`), msg: 'Missing main hook export' },
      { pattern: /useState|useCallback|useMemo/, msg: 'Hook should use React hooks' },
