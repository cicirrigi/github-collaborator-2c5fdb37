    ...
    return [
      {
        ...
        headers: [
          ...
          },
          {
            ...
            value:
              "default-src 'self'; script-src 'self' 'unsafe-eval' 'unsafe-inline' *.vercel-scripts.com js.stripe.com maps.googleapis.com; style-src 'self' 'unsafe-inline' fonts.googleapis.com; font-src 'self' fonts.gstatic.com; img-src 'self' data: blob: *.supabase.co images.unsplash.com; frame-src 'self' *.stripe.com *.stripe.network; connect-src 'self' localhost:3003 *.supabase.co *.stripe.com maps.googleapis.com vitals.vercel-insights.com *.sentry.io *.ingest.sentry.io;",
          },
          // 🎯 Performance & Caching
          {
